...
- **Checklist-first structure**: Kiro-style checkboxes and nested subtasks are
  the default shape
- **Task state markers**: `[ ]` pending, `[~]` in progress, `[/]` partial,
  `[>]` follow-up or routed, `[-]` no-op or deferred, `[?]` review or decision
  needed, `[!]` attention needed, and `[x]` complete and verified. Legacy
  `[Y]`, `[*]`, and `[e]` markers remain readable during migration only.
