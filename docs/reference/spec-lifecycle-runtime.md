...
ordered additional skip signals, including negation. Ignored cache, generated,
or agent-local paths therefore do not become closure evidence; binary files are
also excluded. Keep repository-specific generated roots in those ignore files
instead of expanding a lifecycle-specific hard-coded path list.
