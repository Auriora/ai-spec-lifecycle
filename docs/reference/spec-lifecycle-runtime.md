...
`closure_plan` returns a declarative, agent-usable manifest: the closure
decision, bounded blockers and findings, action handles and dependencies,
affected paths, content hashes and sizes, short previews, reference counts and
actionable samples, and relevant validation commands. It never returns full
replacement file bodies. `detail=section` expands only `references`, `edits`,
or `validation` and requires the same closure inputs plus the expected
`plan_id`. The response has a 32 KiB regression ceiling; that ceiling guards
...
Ignored cache, generated, database, or agent-local paths therefore do not
become closure evidence. Keep other repository-specific generated roots in the
repository ignore files.

`closure_apply` accepts the manifest `plan_id`, closure inputs, and one action
ID rather than accepting a full plan payload. It regenerates executable edits
from current repository state and rejects a stale spec-package fingerprint.
Record rendering is an idempotent spec-ID upsert. Package cleanup operates only
on the package edit and is rejected until matching closure-log and archive-index
records exist, so sequential calls do not duplicate or re-render records. The
tool defaults to dry-run and rejects writes without `write_intent`.
