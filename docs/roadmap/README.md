...
| R010 | now | active | Task state and implementation-readiness tools | B037, B038, active spec `023-task-state-management-tools` | Runtime and MCP can identify stale-open tasks, plan-only completion, blocked-output completion, broad-task splits, and required task context before implementation. | platform | `docs/specs/023-task-state-management-tools/` |
...
1. **Validation plan builder (`019`, R006):** implement `validation_plan` first
   so every later task can ask for the right checks and avoid doc-only skipped
   validation noise.
2. **Task state and context tools (`023`, R010):** implement task listing,
   task detail, context reload, and task-state audit before any write-capable
   state update. This addresses stale-open tasks, broad task ambiguity, and
   plan-only completion.
3. **Evidence quality check (`020`, R007):** classify concrete, vague,
   missing, waived, not-run, and not-applicable evidence using validation-plan
   context.
4. **Closure risk review (`021`, R008):** aggregate closure, promotion,
   validation, evidence, stale-active-doc, and historical-recoverability
   signals before package cleanup.
5. **Staged developer onboarding (`024`):** add `lifecycle_guide`,
   `bootstrap_plan`, and stage readiness once validation and task-state signals
   are available to feed it.
6. **npm publish/release workflow (`022`, R009):** keep release automation
