...
| B057 | active     | Requirement priority labels                              | User request during spec `030-mcp-first-runtime-migration`; requirements authoring friction; active spec `032-requirement-priority-labels`                                  | Promoted to `docs/specs/032-requirement-priority-labels/` in wizard requirements mode. Add a standard priority field or notation for requirements, such as must/should/could or equivalent, so agents can distinguish mandatory closure gates from lower-priority enhancements during design, task planning, validation, and scope negotiation. Decide whether priority belongs in headings, frontmatter, requirement metadata, traceability rows, or lint output. |
| B058 | active     | Canonical context warning noise                          | User discussion of Claude-agent interpretation of `CANONICAL_CONTEXT_MISSING`; B047, spec-local canonical context work; active spec `031-canonical-context-warning-noise`     | Promoted to `docs/specs/031-canonical-context-warning-noise/` in wizard requirements mode. Runtime diagnostics now mark `CANONICAL_CONTEXT_MISSING` advisory/non-blocking and distinguish promotion or closure wording from imported-source authority; source guidance, prompts, templates, durable docs, and bundled copies are being aligned before final verification and closure. |
