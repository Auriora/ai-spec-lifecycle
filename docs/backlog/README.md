...
| B008 | done       | Closure risk review                                      | Archive index entries `013-agent-backed-lifecycle-tools` and `021-closure-risk-review`; `docs/reference/spec-lifecycle-runtime.md`; review packet runtime                   | Runtime and MCP now aggregate closure, promotion, validation, evidence, decision, live-doc, and recovery signals into advisory closure risk output before package closure.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| B031 | done       | Phase gate check                                         | Kiro requirements-design-tasks-implementation workflow review; lifecycle runtime; Spec 033 implementation evidence                                                         | Shipped standalone MCP `phase_gate_check` and retained CLI `phase-gate-check` with eight-phase inference, conservative advancement, bounded compact/full/section output, fingerprint-guarded expansion, adapter provenance, and authoritative-source preservation.                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| B044 | done       | npm publish and release workflow                         | B026 residual risk; user request; archive index entry `022-npm-publish-release-workflow`                                                                                    | Added GitHub Actions CI/CD for validation, packaged release artifacts, guarded npm publish, GitHub release evidence, and install verification.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| B049 | done       | Guided documentation wizard                              | User request for step-by-step documentation, feedback, and open-question guidance; archive index entry `026-guided-documentation-wizard`                                    | Prompt-only v1 added a preview-first guided workflow that asks stage-specific questions, classifies open questions and feedback, and composes existing lifecycle-guide, stage-readiness, prompt, and task-context surfaces instead of creating a second lifecycle engine.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| B050 | active     | Phase completion helper                                  | External agent feedback in `docs/reviews/agent-feedback.md`; B012, B023, B031; active spec `034-phase-completion-helper`                                                    | Promoted to `docs/specs/034-phase-completion-helper/` in wizard requirements mode. Add a structured helper for phase-level completion slices that can update task evidence, verification task tables, evidence logs, readiness status, and grouped task state from one reviewed input. Preserve separate validation and review evidence instead of treating batch updates as proof by themselves.                                                                                                                                                                                                                                                                                                                                    |
...
| B055 | done       | MCP-first runtime migration                              | User discussion about MCP-first agent interfaces, dynamic tool-list compatibility, runtime script retirement, cross-platform packaging spec `028-cross-platform-packaging`, and `../agent-workbench` Node/TS reference | Completed in spec `030-mcp-first-runtime-migration`. Added MCP-first lifecycle guidance, capability introspection, stable-tool-surface fallback behavior, runtime script inventory, migrated `traceability_lookup.py` behavior into MCP-owned internals, removed the migrated executable, and retained `spec_runtime.py` as validation/recovery-only. |
| B056 | done       | Spec closure helper                                      | User request during closure of spec `025-dev-cli-workflow-tools`; B003, B008, B012, B023, B050; archive index entry `029-spec-closure-helper`                               | Completed in spec `029-spec-closure-helper`. Added a preview-first closure helper for the `make durable, cleanup, remove` workflow: durable promotion review, final spec commit capture, closure-log and archive-index rendering, package cleanup planning, cleanup-commit resolution, active-reference checks, follow-up routing metadata, and validation commands. Kept it orchestration-oriented so closure judgment, residual-risk acceptance, and commits remain human or lead-agent decisions.                                                                                                                                                                                                                                    |
| B057 | done       | Requirement priority labels                              | User request during spec `030-mcp-first-runtime-migration`; requirements authoring friction; archive index entry `032-requirement-priority-labels`                            | Completed in spec `032-requirement-priority-labels`. Added requirement-level MoSCoW labels (`must-have`, `should-have`, `could-have`), parser/lint diagnostics, priority-aware readiness and closure coverage, traceability/MCP/agent-context propagation, prompt/template guidance, durable runtime/design docs, bundled plugin parity, and regression coverage. |
| B058 | done       | Canonical context warning noise                          | User discussion of Claude-agent interpretation of `CANONICAL_CONTEXT_MISSING`; B047, spec-local canonical context work; archive index entry `031-canonical-context-warning-noise` | Completed in spec `031-canonical-context-warning-noise`. Runtime diagnostics now mark `CANONICAL_CONTEXT_MISSING` advisory/non-blocking and distinguish promotion or closure wording from imported-source authority; source guidance, prompts, templates, durable docs, and bundled copies were aligned. |
...
| B061 | done       | Deterministic spec ID allocation and creation planning   | Coding-agent session analysis; recurring manual spec-number selection; Spec 035 implementation evidence                                                                        | Delivered per-docs-root archive-aware allocation, scan/no-active/bootstrap integration, traversal-safe preview planning, MCP/CLI parity, evidence fingerprints, and a future atomic-writer boundary. |
| B062 | done       | Compact lifecycle output and invocation provenance       | Coding-agent session analysis across Codex and Claude; high-volume lifecycle calls; Spec 036 implementation and closure evidence                                                | Delivered a versioned compact/full/section contract for phase-gate and spec-creation aggregates, deterministic stale expansion, blocker-preserving bounds, privacy-safe invocation provenance, authoritative package/build identity, MCP/CLI parity, and synchronized Codex/Claude bundles. B025 remains owner of emitted telemetry and remote observability. |
| B063 | done       | Relevant-by-construction closure responses               | User report of oversized `closure_apply` output; ignore-aware scan remediation; spec `037-closure-response-contract`                                                              | Delivered agent-usable closure manifests without replacement bodies, targeted expansion, stateless fingerprint-verified apply, independently scoped actions, idempotent record upserts, guarded package cleanup, ignore/cache/database/WAL/binary exclusions, a 32 KiB manifest ceiling, and a short non-duplicating MCP text envelope. |
| B064 | done       | Lifecycle adoption workflow                              | Qualified bounded Chat Analyser findings from 2026-07-18; B014, B015, B025; Spec 038 delivery evidence                                                                           | Delivered a declarative read-only implementation-start prompt, evidence-before-promotion action routing, MCP-primary recovery separation, ready capability status with bounded in-memory client observation, a 69.2% smaller source skill entrypoint, narrow ordinary-write hooks, quiet removed-cache launchers, repository-local source development, and packaged-only user deployment. The qualified external report receipt preserves unavailable counts and attribution limits. Spec 034 retains phase-completion mutation, Chat Analyser owns analysis repairs, B025 owns telemetry, and B014/B015 remain candidate owners for future bounded discovery and friction reports. |
...
### B058 Canonical Context Warning Noise

- Keep `CANONICAL_CONTEXT_MISSING` advisory unless a separate focused spec
  promotes it to a stronger readiness gate with false-positive handling.
- Make runtime, prompt, and documentation language clear that agents should add
  `canonical-context.md` only when stale docs, imported sources, broad
  durable-doc impact, or canonical-context intent are real implementation
  context risks.
- Avoid telling agents to recreate, modernize, or add artifacts to closed,
  archived, removed, or otherwise historical spec packages.
- Split or clarify `imported-source-risk` signals so phrases such as
  `promotion target` do not make ordinary closure or durable-promotion wording
  look like imported-source authority.
- Candidate surfaces: `lint_spec_package`, `agent_readiness_packet`,
  `closure_check`, prompt guidance, runtime reference docs, and tests.
...
- Delivered standalone MCP `phase_gate_check` and retained recovery/validation
  CLI `phase-gate-check` across requirements, design, tasks, implementation,
  verification, promotion, closure, and unknown phases.
- Advancement is conservative for runnable tasks, stale or missing upstream
  review, unresolved decisions, verification gaps, and promotion/closure proof.
- Compact, full, closed-section, and stale expansion responses preserve
  authoritative source meaning within 20-finding, 10-action, and 32 KiB normal
  bounds.
...
| P1       | B055       | Delivered MCP-first migration work that reduces dual script/MCP contracts and removes migrated runtime scripts by closure.                                                                                                  |
...
| P2       | B057       | Delivered requirement priority semantics so agents can separate must-have gates from should/could enhancements during spec planning and closure.                                                                              |
...
| P2       | B059       | Delivered shared-core extraction so MCP internals no longer call through the retained runtime adapter.                                                                                                                      |
...
| P2       | B031       | Delivered deterministic lifecycle phase transitions that prevent stale requirements, incomplete design, runnable work, or premature promotion and closure.                                                                    |
| P2       | B061       | Removes agent-side spec-number arithmetic while preserving multi-root history, staleness detection, and a safe future writer boundary.                                                                                       |
| P2       | B062       | Establishes the compact aggregate-response and invocation-provenance contract needed before Specs 033 and 035 freeze public schemas; B025 remains the operational telemetry owner.                                           |
