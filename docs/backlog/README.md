...
| B055 | active     | MCP-first runtime migration                              | User discussion about MCP-first agent interfaces, dynamic tool-list compatibility, runtime script retirement, cross-platform packaging spec `028-cross-platform-packaging`, and `../agent-workbench` Node/TS reference | Promoted to `docs/specs/030-mcp-first-runtime-migration/` in wizard requirements mode. Define an MCP-first lifecycle interface, add capability and client-compatibility introspection, support state-aware tools with stable fallback behavior, inventory runtime scripts for migration or retention, and require scripts selected for MCP migration to be removed by spec closure. |
...
| P1       | B055       | Active MCP-first migration work; reduces dual script/MCP contracts and requires explicit removal of migrated runtime scripts by closure.                                                                                    |
