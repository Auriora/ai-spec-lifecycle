...
| B061 | active     | Deterministic spec ID allocation and creation planning   | Coding-agent session analysis; recurring manual spec-number selection; active spec `035-spec-id-allocation-and-creation-plan`                                                  | P2. Promoted to wizard requirements mode. Add per-docs-root archive-aware allocation, scan/no-active orientation fields, traversal-safe creation planning, evidence fingerprints, and a future atomic-writer boundary. |
| B062 | active     | Compact lifecycle output and invocation provenance       | Coding-agent session analysis across Codex and Claude; high-volume lifecycle calls; active spec `036-compact-output-and-invocation-telemetry`                                  | P2. Promoted to wizard requirements mode. Define the versioned compact contract for new aggregates, compatible metadata adoption for existing tools, privacy-safe invocation provenance, deterministic expansion, package/build identity, and transport-boundary ownership. B025 remains owner of emitted telemetry and remote observability. |
