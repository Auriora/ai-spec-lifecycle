...
| B063 | active     | Relevant-by-construction closure responses               | User report of oversized `closure_apply` output; ignore-aware scan remediation; active spec `037-closure-response-contract`                                                       | Make closure MCP calls return only agent-usable decisions, findings, action handles, hashes, sizes, and short previews. Never return replacement file bodies. Regenerate executable plans from repository state, verify `plan_id` fingerprints across restarts, scope actions independently, upsert closure records, exclude ignored/cache/database/WAL/binary scan inputs, and enforce a 32 KiB response regression ceiling. |
