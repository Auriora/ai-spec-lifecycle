...
|--------|------------------|--------|
| `tools/README.md` | Generic tools directory guidance. | Modify to document `slc` once implemented. |
...
| `tools/devcli/pyproject.toml` | Generic `proj-devcli` metadata and `proj` entry point. | Modify to repository-specific metadata and `slc` entry point. |
...
|--------|------|--------|-------|
| Rename CLI package metadata and command entry point. | modify | `tools/devcli/pyproject.toml` | Use `slc` as proposed primary command unless decision changes. |
...
| Add CLI tests. | add | `tests/runtime/test_devcli_*.py` | Mock external commands and avoid user-local mutation. |
