...
Sparse or surprising ranked repository-evidence output is inconclusive. Do not
report that authoritative docs, code, or contracts are absent just because a
search, context, symbol, or other routing surface missed them. State that the
provider query was sparse or partial, then pivot to repository instructions,
durable indexes, targeted file inventory, and direct reads of likely source
files before making absence, implementation, drift, or root-cause claims.
